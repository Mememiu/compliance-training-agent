...
var toConsumableArray = require('../../../_chunks/dep-d26a9931.js');
...
function mergeStringContent(existing, delta) {
  ...
  try {
    ...
    if (_typeof._typeof(existingObj) === "object" && _typeof._typeof(deltaObj) === "object") {
    ...
    if (Array.isArray(existingObj) && Array.isArray(deltaObj)) {
      return JSON.stringify([].concat(toConsumableArray._toConsumableArray(existingObj), toConsumableArray._toConsumableArray(deltaObj)));
    }
    return delta;
  } catch (error) {
...
function isValidEvent(event) {
  return event && _typeof._typeof(event) === "object" && event.type;
}
...
function createActivityContent(activityType, content) {
  var status = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : "complete";
  var strategy = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : "append";
  var deltaInfo = arguments.length > 4 ? arguments[4] : undefined;
...
function createMarkdownContent(data) {
  ...
  var notAssistant = role && role !== "assistant";
  var content = createAIMessageContent(notAssistant ? "".concat(role, "-text") : "markdown", data, status, strategy);
  if (notAssistant) {
    content.ext = _objectSpread(_objectSpread({}, content.ext), {}, {
      role: role
    });
  }
  return content;
...
function updateReasoningData(reasoningData, index, newContent) {
  var updatedData = toConsumableArray._toConsumableArray(reasoningData);
...
function addToReasoningData(reasoningData, newContent) {
  var updatedData = [].concat(toConsumableArray._toConsumableArray(reasoningData), [newContent]);
  return {
    data: updatedData,
  ...
  reasoningMessages.forEach(function (msg) {
    if (msg.role === "assistant") {
      ...
      if (msg.content) {
        reasoningData.push({
          type: "text",
          // 在 reasoning 内部，通常用 text 而不是 markdown
          data: msg.content,
          status: "complete"
        });
      }
...
exports.addToReasoningData = addToReasoningData;
exports.buildToolCallMap = buildToolCallMap;
exports.convertReasoningMessages = convertReasoningMessages;
exports.createAIMessageContent = createAIMessageContent;
exports.createActivityContent = createActivityContent;
exports.createMarkdownContent = createMarkdownContent;
exports.createReasoningContent = createReasoningContent;
exports.createSuggestionContent = createSuggestionContent;
exports.createTextContent = createTextContent;
exports.createThinkingContent = createThinkingContent;
exports.createToolCallContent = createToolCallContent;
exports.extractStateKeyFromDelta = extractStateKeyFromDelta;
...
exports.handleMessagesSnapshot = handleMessagesSnapshot;
exports.handleSuggestionToolCall = handleSuggestionToolCall;
exports.isValidEvent = isValidEvent;
exports.mergeStringContent = mergeStringContent;
exports.parseReasoningContent = parseReasoningContent;
exports.parseSSEData = parseSSEData;
exports.processReasoningContent = processReasoningContent;
exports.processToolCalls = processToolCalls;
exports.updateReasoningData = updateReasoningData;
