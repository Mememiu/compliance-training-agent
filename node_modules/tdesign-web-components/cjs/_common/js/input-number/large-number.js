...
function isInputNumber(num) {
  if (!num) return true;
  if (lodashEs.isNumber(num)) return !Number.isNaN(num);
...
function decimalPlacesToFixedNum(num, decimalPlaces) {
  if (lodashEs.isObject(decimalPlaces)) {
...
function largeNumberToFixed(number) {
  ...
  var places = lodashEs.isObject(decimalPlaces) ? decimalPlaces.places : decimalPlaces;
  var enableRound = lodashEs.isObject(decimalPlaces) ? (_decimalPlaces$enable2 = decimalPlaces.enableRound) !== null && _decimalPlaces$enable2 !== void 0 ? _decimalPlaces$enable2 : true : true;
  if (!lodashEs.isString(number)) return String(number);
