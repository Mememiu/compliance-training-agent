...
function canAddNumber(num, max) {
  ...
  if (largeNumber && lodashEs.isString(num)) {
...
function putInRangeNumber(val, params) {
  ...
  if (largeNumber && (lodashEs.isString(max) || max === Infinity) && (lodashEs.isString(min) || min === -Infinity)) {
...
function getStepValue(p) {
  ...
  var tStep = lodashEs.isNumber(step) ? String(step) : step;
  var newVal;
  if (op === "add") {
    if (largeNumber && lodashEs.isString(lastValue)) {
  ...
  if (lodashEs.isUndefined(lastValue)) {
...
function getMaxOrMinValidateResult(p) {
  var largeNumber = p.largeNumber,
    ...
    max = p.max,
    min = p.min;
  if (lodashEs.isUndefined(value) || lodashEs.isUndefined(largeNumber)) return void 0;
  if (largeNumber && lodashEs.isNumber(value)) {
