...
function getUniqueRowValue(row, colKey) {
  ...
  var rowValue = lodashEs.get(row, colKey);
  ...
  return createClass._createClass(TableTreeStore, [{
    ...
    value: function getAllUniqueKeys(data, keys) {
      ...
      for (var i = 0, len = data.length; i < len; i++) {
        ...
        var children = lodashEs.get(item, keys.childrenKey);
    ...
    value: function toggleExpandData(p, dataSource, keys, type) {
      ...
      var rowValue = lodashEs.get(p.row, keys.rowKey);
      if (lodashEs.isUndefined(rowValue)) {
        _common_js_log_log["default"].error("EnhancedTable", "`rowKey` could be wrong, can not get rowValue from `data` by `rowKey`.");
        return [];
      }
      var childrenNodes = lodashEs.get(p.row, keys.childrenKey);
    ...
    value: function updateExpandRow(changeRow, dataSource, keys) {
      ...
      var childrenNodes = lodashEs.get(row, keys.childrenKey);
    ...
    value: function updateData(rowValue, newRowData, dataSource, keys) {
      ...
      if (lodashEs.get(newRowData, keys.childrenKey) === true) {
        var oldChildren = lodashEs.get(rowState.row, keys.childrenKey);
      ...
      if (rowState.parent) {
        var siblings = lodashEs.get(rowState.parent.row, keys.childrenKey);
    ...
    value: function remove(key, dataSource, keys) {
      ...
      if (r && r.rowIndex >= 0) {
        ...
        if (r.parent) {
          var siblings = lodashEs.get(r.parent.row, keys.childrenKey);
          var index = siblings.findIndex(function (item) {
            return lodashEs.get(item, keys.rowKey) === key;
          });
    ...
    value: function removeChildren(key, dataSource, keys) {
      ...
      if (r && r.rowIndex >= 0) {
        ...
        r.expanded = false;
        lodashEs.set(r.row, keys.childrenKey, void 0);
    ...
    value: function appendTo(rowValue, newData, dataSource, keys) {
      ...
      var children = lodashEs.get(state.row, keys.childrenKey);
    ...
    value: function appendToRoot(newData, dataSource, keys) {
      ...
      for (var i = 0, len = tmpNewData.length; i < len; i++) {
        var rowValue = lodashEs.get(tmpNewData[i], keys.rowKey);
        if (!rowValue) {
    ...
    value: function insert(rowValue, newData, dataSource, keys, type) {
      var state = this.treeDataMap.get(rowValue);
      if (!this.validateDataExist(state, rowValue)) return dataSource;
      var newRowValue = lodashEs.get(newData, keys.rowKey);
      var mapState = this.treeDataMap.get(newRowValue);
      ...
      if (state.parent) {
        var childrenIndex = state.parent.row[keys.childrenKey].findIndex(function (t) {
          return rowValue === lodashEs.get(t, keys.rowKey);
        });
    ...
    value: function swapData(dataSource, params, keys) {
      ...
      var startRowValue = lodashEs.get(params.current, keys.rowKey);
      var endRowValue = lodashEs.get(params.target, keys.rowKey);
      ...
      if (startState.parent) {
        ...
        for (var i = 0, len = children.length; i < len; i++) {
          if (lodashEs.get(children[i], keys.rowKey) === startRowValue) {
            currentIndex = i;
            count += 1;
          }
          if (lodashEs.get(children[i], keys.rowKey) === endRowValue) {
            targetIndex = i;
            count += 1;
          }
          if (count >= 2) break;
    ...
    value: function expandAll(dataSource, keys) {
      ...
      var expandLoop = function expandLoop(dataSource2, keys2) {
        ...
        for (var i = 0, len = dataSource2.length; i < len; i++) {
          ...
          var rowValue = lodashEs.get(item, keys2.rowKey);
          var state = _this3.treeDataMap.get(rowValue);
          var children = lodashEs.get(item, keys2.childrenKey);
    ...
    value: function foldAll(dataSource, keys) {
      ...
      for (var i = 0, len = dataSource.length; i < len; i++) {
        ...
        var rowValue = lodashEs.get(item, keys.rowKey);
        var state = this.treeDataMap.get(rowValue);
...
function updateRowExpandLength(treeDataMap, row, distance, type, keys) {
  ...
  while (tmp) {
    var _state$parent;
    var state = treeDataMap.get(lodashEs.get(tmp, keys.rowKey));
...
function clearRowExpandLength(treeDataMap, row, keys) {
  var children = lodashEs.get(row, keys.childrenKey);
  if (children !== null && children !== void 0 && children.length) {
    children.forEach(function (item) {
      var state = treeDataMap.get(lodashEs.get(item, keys.rowKey));
      if (!state) return;
      state.expandChildrenLength = 0;
...
function updateRowData(data, key, newData, keys) {
  for (var i = 0, len = data.length; i < len; i++) {
    var item = data[i];
    if (lodashEs.get(item, keys.rowKey) === key) {
      data[i] = newData;
      return;
    }
    var children = lodashEs.get(item, keys.childrenKey) || [];
...
function updateRowIndex(treeDataMap, dataSource, extra) {
  ...
  for (var rowIndex = start; rowIndex < end; rowIndex++) {
    ...
    var state = treeDataMap.get(lodashEs.get(item, extra.rowKey));
    if (!state) {
