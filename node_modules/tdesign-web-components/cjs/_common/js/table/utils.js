...
function isRowSelectedDisabled(selectColumn, row, rowIndex) {
  ...
  var disabled = lodashEs.isFunction(selectColumn.disabled) ? selectColumn.disabled({
    row: row,
    rowIndex: rowIndex
  }) : selectColumn.disabled;
  if (selectColumn.checkProps) {
    if (lodashEs.isFunction(selectColumn.checkProps)) {
...
function getColWidthAttr(col, attrKey) {
  ...
  return lodashEs.isNumber(attr) ? attr : parseFloat(attr);
}
function getEditableKeysMap(keys, list, rowKey) {
  var map = {};
  for (var i = 0, len = list.length; i < len; i++) {
    var rowValue = lodashEs.get(list[i], rowKey);
