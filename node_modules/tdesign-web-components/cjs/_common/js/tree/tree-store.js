...
var mitt = require('../../../_chunks/dep-707b7747.js');
...
var TreeStore = /*#__PURE__*/function () {
  ...
  function TreeStore(options) {
    ...
    this.hasFilter = lodashEs.isFunction(config.filter);
  ...
  return createClass._createClass(TreeStore, [{
    ...
    value: function getNode(item) {
      var node = null;
      if (lodashEs.isString(item) || lodashEs.isNumber(item)) {
    ...
    value: function getNodes(item, options) {
      ...
      if (options) {
        ...
        if (lodashEs.isNumber(conf.level) && conf.level !== Infinity) {
          nodes = nodes.filter(function (node) {
            return node.level <= conf.level;
          });
        }
        if (lodashEs.isFunction(conf.filter)) {
          nodes = nodes.filter(function (node) {
            var nodeModel = node.getModel();
            return conf.filter(nodeModel);
          });
        }
        if (lodashEs.isPlainObject(conf.props)) {
          nodes = nodes.filter(function (node) {
    ...
    value: function parseNodeData(para, item) {
      ...
      var node = null;
      var data = null;
      if (lodashEs.isString(para) || lodashEs.isNumber(para)) {
        value = para;
    ...
    value: function appendNodes(para, item) {
      ...
      if (spec.data) {
        if (!spec.node) {
          ...
          } else if (lodashEs.isArray(spec.data)) {
    ...
    value: function updated(node) {
      ...
      this.updateTick = nextTick(function () {
        ...
        var stateId = "t".concat(( /* @__PURE__ */new Date()).getTime());
    ...
    value: function replaceExpanded(list) {
      ...
      var added = lodashEs.difference(list, expanded);
      var removed = lodashEs.difference(expanded, list);
    ...
    value: function emit(name, state) {
      ...
      var methodName = lodashEs.camelCase("on-".concat(name));
    ...
    value: function lockFilterPathNodes() {
      ...
      if (!currentFilter || !lodashEs.isFunction(currentFilter)) return;
