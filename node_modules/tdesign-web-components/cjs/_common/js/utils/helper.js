...
function removeEmptyAttrs(obj) {
  ...
  Object.keys(obj).forEach(function (key) {
    if (!lodashEs.isUndefined(obj[key]) || lodashEs.isNull(obj[key])) {
      newObj[key] = obj[key];
    }
  });
  return newObj;
...
function getBackgroundColor(color) {
  if (lodashEs.isString(color)) {
    return color;
  }
  if (lodashEs.isArray(color)) {
...
function getCharacterLength(str, maxCharacter) {
  var hasMaxCharacter = lodashEs.isNumber(maxCharacter);
  if (!str || str.length === 0) {
    if (hasMaxCharacter) {
      return {
        length: 0,
...
function pxCompat(param) {
  return lodashEs.isNumber(param) ? "".concat(param, "px") : param;
}
