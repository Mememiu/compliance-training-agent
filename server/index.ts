...
app.get("/api/courses", (req, res) => {
  try {
    ...
    res.status(500).json({ error: error?.message || "获取课程失败" });
  }
});
...
app.get("/api/training/progress", (req, res) => {
  try {
    ...
    const progress = db.getAllProgress(userId);
    res.json({ progress });
  } catch (error: any) {
...
app.post("/api/training/progress/:courseId", (req, res) => {
  try {
    ...
    // 如果状态变为 in_progress 且没有 started_at，设置 started_at
    if (status === 'in_progress') {
      const existing = db.getCourseProgress(userId, courseId);
    ...
    const result = db.upsertProgress({ user_id: userId, course_id: courseId, ...updates });
    res.json({ progress: result });
  } catch (error: any) {
    res.status(500).json({ error: error?.message || "更新进度失败" });
  }
});
...
app.post("/api/training/quiz/:courseId/submit", (req, res) => {
  try {
    ...
    const passed = score >= course.passThreshold ? 1 : 0;
    ...
    db.saveQuizResult({
      ...
      score,
      passed,
      total_questions: course.quiz.length,
    ...
    res.status(500).json({ error: error?.message || "提交测验失败" });
  }
});

// 获取测验历史记录
app.get("/api/training/quiz/:courseId/history", (req, res) => {
  try {
    const { courseId } = req.params;
    const userId = (req.query.userId as string) || 'default';
    const results = db.getQuizResults(userId, courseId);
    res.json({ results });
  } catch (error: any) {
...
// 获取培训仪表盘统计数据
app.get("/api/training/dashboard", (req, res) => {
  try {
    const userId = (req.query.userId as string) || 'default';
    const allProgress = db.getAllProgress(userId);
    
    const totalCourses = COURSES.length;
    const completedCourses = allProgress.filter(p => p.status === 'completed' && p.passed === 1).length;
    ...
    const avgScore = allProgress.filter(p => p.score !== null).length > 0
      ? Math.round(allProgress.filter(p => p.score !== null).reduce((sum, p) => sum + (p.score || 0), 0) / allProgress.filter(p => p.score !== null).length)
      : 0;
    
    // 各课程进度详情
    const courseProgress = COURSES.map(c => {
      const p = allProgress.find(ap => ap.course_id === c.id);
      return {
        courseId: c.id,
        title: c.title,
        category: c.category,
