...
export function CourseMicrocourse() {
  ...
  return (
    <section className="flex-1 overflow-y-auto" aria-label={`${config.title}互动微课`}>
      <div className="px-4 py-4 lg:px-8 flex flex-wrap items-center justify-between gap-3" style={{ borderBottom: '1px solid var(--td-component-stroke)' }}>
      ...
      <iframe key={token} ref={frame} src={`${import.meta.env.BASE_URL}microcourses/${config.asset}/index.html`} title={`${config.title}互动微课`} onLoad={init} onError={() => setBridgeError('微课文件未能载入，请重试。')} allow="fullscreen" style={{ display: 'block', width: '100%', height: 'calc(100dvh - 170px)', minHeight: 600, border: 0, background: '#f4f1eb' }} />
