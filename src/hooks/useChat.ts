...
function buildKnowledgeBase(): KnowledgeEntry[] {
  ...
  entries.push({
    ...
    answer: `## 培训课程概览\n\n培训室将课程按主题组织，当前包含以下三个模块：\n\n1. **数据隐私与信息安全** — 个人信息保护、数据处理规范、账户安全与社会工程学防范\n2. **反腐败与商业道德** — 反贿赂法律、利益冲突管理、廉洁行为准则\n3. **劳动合规与职场行为** — 劳动合同权益、职场反骚扰与平等就业\n\n每个课程模块包含多个课时学习和测验考核，及格线为80分。请在培训首页开始学习。`,
  });
...
function searchKnowledgeBase(query: string): string {
  ...
  // 默认回复
  return `您好！我是企业学习助手。关于您的问题"${query}"，我目前的课程知识库中没有找到完全匹配的内容。\n\n我可以帮您解答以下方面的问题：\n\n- **数据隐私与信息安全** — 个人信息保护、数据分类分级、密码安全与钓鱼防范\n- **反腐败与商业道德** — 反贿赂法律、利益冲突、廉洁行为准则\n- **劳动合规与职场行为** — 劳动合同、加班费、职场反骚扰\n\n请尝试更具体地描述您的问题，或学习相关课程获取详细内容。`;
}
